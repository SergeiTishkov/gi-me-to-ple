    ...
    <div className="bg-blue-950 text-white min-h-screen p-4">
      ...
      {menuOpen && (
        <div className="bg-blue-900 p-4 rounded mb-6">
          <button className="bg-blue-700 px-4 py-2 rounded hover:bg-blue-600 w-full">
            Login / Register
          </button>
        </div>
      )}
      ...
      <main className="space-y-4">
        {requests.map((req) => (
          <div
            key={req.id}
            className="bg-blue-800 rounded-xl p-4 shadow-lg space-y-2"
          >
        ...
        {requests.length === 0 && (
          <p className="text-center text-blue-300">Loading requests...</p>
        )}
      </main>
    </div>
  );
